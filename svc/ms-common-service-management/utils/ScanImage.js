...
const highPassFilter = (src, kSize) => {
  ...
  const scalar = new cv.Scalar(127, 127, 127)
  const newKernel = new cv.Mat(src.rows, src.cols, cv.CV_32FC3, scalar)
...
const blackPointSelect = (src, blackPoint = 66) => {
  let dst = new cv.Mat()
  const whitePoint = blackPoint > 254 ? 254 : blackPoint < 0 ? 0 : blackPoint
  const scalar = new cv.Scalar(blackPoint, blackPoint, blackPoint, blackPoint)
  const kernel = new cv.Mat(src.rows, src.cols, src.type(), scalar)
...
const blackAndWhite = (src) => {
  ...
  const subA = new cv.Mat()
  const subB = new cv.Mat()
...
export const scanImage = (src, mode, blackPoint, whitePoint, kSize = 51) => {
  let dst = src//.clone()
  if (mode === 'rmode') {
