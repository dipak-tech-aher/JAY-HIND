  ...
  try {
    ...
    if (Array.isArray(userPermissions)) {
      ...
      if (hasPermission && accessType) {
        if (method === 'POST' && (accessType === 'write' || accessType === 'read')) {
          hasAccess = true
      ...
      if (!hasAccess) {
        return responseHelper.sendResponse(req, res, { status: statusCodeConstants.ACCESS_FORBIDDEN, message: 'Access forbidden' })
      }
    } else {
      return responseHelper.sendResponse(req, res, { status: statusCodeConstants.ACCESS_FORBIDDEN, message: 'Access forbidden' })
    }
    ...
    return responseHelper.sendResponse(req, res, { status: statusCodeConstants.ERROR, message: 'Internal server error' })
  }
}
