module.exports = function (sequelize, DataType) {
  const BcaeAppConfig = sequelize.define('BcaeAppConfig', {
    ...
    maxRefreshSessionTimeout: {
      type: (sequelize.options.dialect === 'mssql') ? DataType.STRING : DataType.JSONB,
      get: function () {
        return sequelize.options.dialect === 'mssql' ? JSON.parse(this.getDataValue('maxRefreshSessionTimeout')) : this.getDataValue('maxRefreshSessionTimeout')
      },
      set: function (value) {
        return sequelize.options.dialect === 'mssql' ? this.setDataValue('maxRefreshSessionTimeout', JSON.stringify(value)) : this.setDataValue('maxRefreshSessionTimeout', value)
      }
    },
    maxPasswordRetry: {
      type: DataType.INTEGER
