...
//import { conn.Workflow, WorkflowTxn, WorkflowHdr, sequelize, Role, BusinessEntity, BusinessUnit, Interaction } from '../model'
...
const executeCollectInputTask = async (collectInputTask, inputContext, t, activityPrefix, taskActivityPrefix, mobileNumber, msg, txnData, nextActivityId, tranId, conn) => {
    ...
    for (const assignment of manualTask.assignments) {
      ...
      if (condition) {
        if (assignment.assignmentType === 'BYHIERARCHY') {
          ...
          const hasRecord = await WorkflowTxn.findOne({ where: { wfHdrId: inputContext.wfHdrId, activityId, taskId } })
          if (hasRecord) {
            wfTxnData = {
...
const executePost = async (task, inputContext, t, activityPrefix, taskActivityPrefix, hasMoreTask, datas, tranId, conn) => {
  logger.debug('Executing POST')
  const properties = findUrlParamsAndBody(task, inputContext)
  console.log('Properties in WA engine', properties)
  const response = await Got.post({
    headers: { 'content-type': 'application/json', 'x-tenant-id': tranId },
    url: properties.url + `?tenant-id=${tranId}`,
  ...
  console.log('Response of WA engine :', response.body)
  if (response.body) {
...
export const assignWFToEntity = async (entityId, entity, workflowId, conn) => {
  ...
  if (workflowHdr) {
    ...
    if (workflowTxns.length > 0) {
      if (workflowTxns[0].wfTxnStatus === 'USER_WAIT') {
        ...
        const workflowDfn = await conn.Workflow.findOne({ where: { workflowId: workflowHdr.wfDefnId } })
...
const updateContextWhatsApp = async (data, inputContext, t, activityPrefix, taskActivityPrefix, hasMoreTask, conn) => {
  ...
  // console.log('wfDatawfData--->', wfData)
  await conn.WorkflowHdr.update(wfData, { where: { wfHdrId: inputContext.wfHdrId }, transaction: t })
  await t.commit()
}

export const processWhatsAppStartStep = async (wfHdr, wfJson, source, conn) => {
  logger.debug('Performing start step for new record')
  try {
    const process = wfJson.definitions.process
    if (process) {
...
export const continueChatWFExecution = async (wfJson, currentActivityId, inputContext, mobileNumber, msg, tranId, conn) => {
    ...
    if (hasWaitRecord) {
      ...
      if (currentActivity.type === 'END') {
        ...
        if (decision) {
          ...
          //console.log('here also ', nextActivityId, inputContext, mobileNumber, msg, tranId)
          await t.commit()
          return await continueChatWFExecution(wfJson, nextActivityId, inputContext, mobileNumber, msg, tranId, conn)
        } else {
          await t.commit()
            ...
            if (hasWaitRecord) {
              ...
              if (!hasTask) {
                ...
                if (task.type === 'SENDMESSAGE') {
                  ...
                  const executeCollectInputTaskResult = await executeCollectInputTask(task, inputContext, t, activityPrefix, taskActivityPrefix, mobileNumber, msg, txnData, nextActivityId, tranId, conn)
                  await conn.WorkflowTxn.destroy({
                    where: {
                      wfHdrId: inputContext.wfHdrId
                    },
                  ...
                  return await executeAPITaskForWhatsApp(task, inputContext, t, activityPrefix, taskActivityPrefix, mobileNumber, msg, txnData, nextActivityId, tranId, conn)
...
export const executeAPITaskForWhatsApp = async (APITask, context, t, activityPrefix, taskActivityPrefix, mobileNumber, msg, txnData, nextActivityId, tranId, conn) => {
  logger.debug('Executing API Task')
  if (APITask.api.method === 'POST') {
    await executePost(APITask, context, t, activityPrefix, taskActivityPrefix, nextActivityId, txnData, tranId, conn)
  } else if (APITask.api.method === 'PUT') {
    await executePut(APITask, context, t, activityPrefix, taskActivityPrefix, nextActivityId, txnData, conn)
  } else if (APITask.api.method === 'GET') {
    return await executeGet(APITask, context, t, activityPrefix, taskActivityPrefix, nextActivityId, txnData, conn)
  } else if (APITask.api.method === 'DELETE') {
    await executeDelete(APITask, context, t, activityPrefix, taskActivityPrefix, nextActivityId, txnData, conn)
  } else {
    logger.debug('No API Task found')
  }
  logger.debug('Successfully Executed API Task')
