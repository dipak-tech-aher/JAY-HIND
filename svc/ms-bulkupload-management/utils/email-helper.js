import { logger, defaultStatus } from '@utils'
...
export class EmailHelper {
  async sendMail (options) {
    try {
      ...
      let smtpInfo = await conn.BcaeAppConfig.findOne({ where: { status: defaultStatus.ACTIVE } })
      ...
      transporter.sendMail({
        ...
        logger.debug("Mail send Error ===> ", err);
        logger.debug("Mail send Success ===> ", info);
        if (err) {
