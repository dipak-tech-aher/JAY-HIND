...
const LiveOverview = (props) => {
    ...
    const getOverView = () => {
        axios.post(properties.API_ENDPOINT + properties.INTERACTION_API + '/live-interactions-by-status', { ...searchParams, fromDate: moment().format('YYYY-MM-DD'), toDate: moment().format('YYYY-MM-DD HH:mm:ss') }, {
            headers: {
    ...
    useEffect(() => {
        ...
        const uniqueHours = [...new Set(chartData.map(item => new Date(item.createdAt).getUTCHours()))];
        ...
        chartData.forEach(item => {
            const createdAtHour = new Date(item.createdAt).getUTCHours();
            const status = item.intxnStatus;
